...
function mixinMigration(MsSQL) {
  MsSQL.prototype.showFields = function showFields(model, cb) {
  ...
  MsSQL.prototype.showIndexes = function showIndexes(model, cb) {
  ...
  MsSQL.prototype.autoupdate = function autoupdate(models, cb) {
    var self = this;
  ...
  MsSQL.prototype.isActual = function isActual(models, cb) {
    var ok = true;
    var self = this;
  ...
  MsSQL.prototype.alterTable = function alterTable(model, actualFields, actualIndexes, done, checkOnly) {
    var self = this;
    ...
    var sqlColumnNames = [];
    Object.keys(m.properties).forEach(function(name) {
      var property = m.properties[name];
      if (property) {
        if (property.mssql && property.mssql.columnName) {
          sqlColumnNames.push(property.mssql.columnName);
        } else {
          sqlColumnNames.push(property.name);
        }
      }
    });
    ...
    propNames.forEach(function(propName) {
      ...
      var found;
      var sqlColumnName = self.column(model, propName);
      actualFields.forEach(function(f) {
        if (f.Field === sqlColumnName) {
          found = f;
        }
      });

      if (found) {
        actualize(sqlColumnName, found);
      } else {
        columnsToAdd.push(sqlColumnName + ' ' + self.propertySettingsSQL(model, propName));
      }
    });
    ...
    actualFields.forEach(function(f) {
      ...
      var notFound = !~sqlColumnNames.indexOf(f.Field);
      if (notFound) {
    ...
    propNames.forEach(function(propName) {
      ...
      if (!found) {
        ...
        if (kind && type) {
          sql.push('ADD ' + kind + ' INDEX ' + columnName + ' (' + columnName + ') ' + type);
        } else {
  ...
  MsSQL.prototype.propertiesSQL = function propertiesSQL(model) {
        // debugger;
    var self = this;
  ...
  MsSQL.prototype.singleIndexSettingsSQL = function singleIndexSettingsSQL(model, prop, add) {
  ...
  MsSQL.prototype.indexSettingsSQL = function indexSettingsSQL(model, prop) {
  ...
  MsSQL.prototype.propertySettingsSQL = function propertySettingsSQL(model, prop) {
    var p = this._models[model].properties[prop];
    return this.columnDataType(model, prop) + ' ' +
            (isNullable(p) ? 'NULL' : 'NOT NULL');
  };

  MsSQL.prototype.automigrate = function automigrate(models, cb) {
    var self = this;
    if ((!cb) && ('function' === typeof models)) {
      cb = models;
      models = undefined;
  ...
  MsSQL.prototype.dropTable = function dropTable(model, cb) {
    var tblName = this.tableEscaped(model);
  ...
  MsSQL.prototype.createTable = function createTable(model, cb) {
    var tblName = this.tableEscaped(model);
  ...
  MsSQL.prototype.createIndexes = function createIndexes(model) {
    var self = this;
    var sql = [];
  ...
  MsSQL.prototype.columnDataType = function columnDataType(model, property) {
